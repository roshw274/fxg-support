...
module.exports = async (message) => {
  ...
  // Check if removal would result in negative points
  if (staff.points - removePoints < 0) {
    return message.reply(`❌ Cannot remove ${removePoints} points. Staff only has ${staff.points} points.`);
  }
  ...
  try {
    ...
    console.log(error);
    return message.reply("An Error Occurred While Removing Points.");
  }
  ...
  if (channelPLog) {
    const logEmbed = new EmbedBuilder()
      .setDescription(`<@${staffId}> - -${removePoints}`)
      .setColor("Random");
