...
module.exports = async (message) => {
  ...
  if (!staff) {
    ...
    const staffPEmbed = new EmbedBuilder()
      ...
      .setDescription(`Current Points Of <@${newStaff.staffId}>`) // 
      .addFields({
        name: "Points:",
        value: `${newStaff.points}`, // 
      })
      .setColor("Random");
    ...
    if (channelPLog) {
      const logEmbed = new EmbedBuilder()
        .setDescription(`<@${newStaff.staffId}> - +${addPoints}`)
        .setColor("Random");
  ...
  try {
    ...
    console.log(error);
    return message.reply("An Error Occurred While Adding Points.");
  }
  const staffPEmbed = new EmbedBuilder()
    .setTitle(`Points Of Staff:`)
  ...
  if (channelPLog) {
    const logEmbed = new EmbedBuilder()
      .setDescription(`<@${staffId}> - +${addPoints}`)
      .setColor("Random");
    channelPLog.send({ embeds: [logEmbed] }).catch((error) => {
      console.error("Failed to send to points log:", error);
      message.channel.send("⚠️ Points updated but failed to log to points channel.").catch(() => {});
